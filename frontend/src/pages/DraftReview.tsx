...
export default function DraftReview() {
  ...
  const fetchDraft = async () => {
    try {
      ...
      // If draft just finished generating, show success message
      if (draft?.status === 'generating' && response.data.status === 'ready') {
  ...
  const handleRegenerate = async () => {
    try {
      ...
      const response = await apiClient.post(`/api/drafts/${draftId}/regenerate`, {
        topic_count: null,
      ...
      if (response.data.status === 'ready') {
        ...
        // Regeneration failed
        setError(response.data.error || response.data.message || 'Failed to regenerate draft')
      } else {
        // Unknown status
        setError(`Unexpected status: ${response.data.status}`)
      }
    } catch (err: any) {
      setError(err.response?.data?.detail || err.response?.data?.message || 'Failed to regenerate draft')
    } finally {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
      ...
      <div className="mb-6">
        ...
        <div className="flex items-center gap-2 text-sm text-gray-600 mb-4">
          <span className={`px-3 py-1 rounded-full text-xs font-medium ${
            ...
            draft.status === 'editing' ? 'bg-blue-100 text-blue-800' :
      ...
      {draft.status === 'generating' && (
        <div className="bg-blue-50 border-l-4 border-blue-500 p-4 mb-6">
          <div className="flex items-center">
            ...
            <div>
              <h3 className="text-sm font-medium text-blue-800">Generating Draft...</h3>
              <p className="mt-1 text-sm text-blue-700">
                Your newsletter is being generated. This may take 10-30 seconds. 
          ...
          <div className="flex">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-yellow-700">
                This draft is empty because no content was found from your sources. 
                Please connect content sources and crawl them, then regenerate this draft.
              </p>
            </div>
          </div>
        </div>
      ...
      {draft.metadata?.fallback && !draft.metadata?.no_content && (
        <div className="bg-blue-50 border-l-4 border-blue-400 p-4 mb-6">
          <div className="flex">
            ...
            <div>
              <h3 className="text-sm font-medium text-blue-800">Limited Content</h3>
              <p className="mt-1 text-sm text-blue-700">
                No trending topics were detected. This draft shows recent content summaries instead. 
                For better results, ensure your sources have diverse, recent content.
              </p>
            </div>
          </div>
        </div>
      ...
      {/* Metadata */}
      {draft.metadata.trends_used && draft.metadata.trends_used.length > 0 && (
        <Card className="mt-6">
          <CardHeader>
          ...
          <CardContent>
            <div className="flex flex-wrap gap-2">
              {draft.metadata.trends_used.map((trend: string, index: number) => (
                <span
                  key={index}
      ...
      <Dialog open={showPublishDialog} onOpenChange={setShowPublishDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to publish this draft? It will be sent to your email address.
              This action will mark the draft as published.
            </DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={showRegenerateDialog} onOpenChange={setShowRegenerateDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to regenerate this draft? Your current edits will be lost
              and a new draft will be created with fresh content.
            </DialogDescription>
          </DialogHeader>
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              The draft will be permanently removed from your account.
            </DialogDescription>
          </DialogHeader>
