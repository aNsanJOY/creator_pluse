"""
Groq API Usage Tracking Routes
Endpoints for viewing Groq usage statistics and rate limits
"""
...
@router.get("/stats")
async def get_groq_usage_stats(
    days: int = Query(30, ge=1, le=365, description="Number of days to look back"),
    current_user: dict = Depends(get_current_active_user)
):
    """
    Get Groq API usage statistics for the current user
    
    Query Parameters:
    - days: Number of days to look back (1-365, default 30)
    
    Returns:
    - total_calls: Total number of Groq API calls
    - total_tokens: Total tokens used
    - total_prompt_tokens: Total input tokens
    - total_completion_tokens: Total output tokens
    - by_model: Breakdown by Groq model
    - by_day: Daily breakdown
    """
    try:
        user_id = current_user["id"]
        
        stats = await groq_usage_tracker.get_usage_stats(
            user_id=user_id,
            days=days
        )
        ...
        raise HTTPException(
            ...
            detail=f"Failed to fetch Groq usage stats: {str(e)}"
        )


@router.get("/rate-limits")
async def get_groq_rate_limits(
    current_user: dict = Depends(get_current_active_user),
    supabase: Client = Depends(get_supabase)
):
    """
    Get current Groq rate limits for the user
    ...
    try:
        ...
        result = supabase.table("groq_rate_limits").select("*").eq("user_id", user_id).execute()
        
        if not result.data:
            # Create default rate limits
            await groq_usage_tracker.check_rate_limit(user_id, "minute")
            await groq_usage_tracker.check_rate_limit(user_id, "day")
            result = supabase.table("groq_rate_limits").select("*").eq("user_id", user_id).execute()
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to fetch Groq rate limits: {str(e)}"
        )


@router.get("/rate-limit/{limit_type}")
async def get_groq_rate_limit(
    limit_type: str,
    current_user: dict = Depends(get_current_active_user)
):
    """
    Check Groq rate limit for a specific time window
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to check Groq rate limit: {str(e)}"
        )


@router.get("/logs")
async def get_groq_usage_logs(
    ...
    """
    Get detailed Groq API usage logs
    
    Query Parameters:
    - limit: Number of logs to return (1-500, default 50)
    - offset: Offset for pagination
    
    Returns paginated list of Groq API call logs
    """
    try:
        user_id = current_user["id"]
        
        # Get total count
        count_result = supabase.table("groq_usage_logs").select("*").eq("user_id", user_id).execute()
        total_count = len(count_result.data) if count_result.data else 0
        
        # Get paginated results
        result = supabase.table("groq_usage_logs").select("*").eq(
            "user_id", user_id
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to fetch Groq usage logs: {str(e)}"
        )


@router.get("/summary")
async def get_groq_usage_summary(
    current_user: dict = Depends(get_current_active_user)
):
    """
    Get a quick summary of Groq API usage
    ...
    try:
        ...
        # Get today's stats
        today_stats = await groq_usage_tracker.get_usage_stats(
            user_id=user_id,
            days=1
        )
        
        # Get this month's stats
        month_stats = await groq_usage_tracker.get_usage_stats(
            user_id=user_id,
            days=30
        )
        
        # Get rate limits
        minute_limit = await groq_usage_tracker.check_rate_limit(
            user_id=user_id,
            limit_type="minute"
        )
        
        day_limit = await groq_usage_tracker.check_rate_limit(
            user_id=user_id,
            limit_type="day"
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to fetch Groq usage summary: {str(e)}"
        )
