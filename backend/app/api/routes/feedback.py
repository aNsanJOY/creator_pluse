...
async def submit_feedback(
    ...
    try:
        ...
        draft_result = (
            supabase.table("newsletter_drafts")
            .select("id")
            .eq("id", feedback_data.newsletter_id)
            .eq("user_id", user_id)
            .execute()
        )
        ...
        old_feedback = (
            ...
            .select("*")
            .eq("newsletter_id", feedback_data.newsletter_id)
            .eq("section_id", feedback_data.section_id)
            .eq("user_id", user_id)
            .execute()
        )

        if old_feedback.data:
            update = supabase.table("feedback").update(
                {
                    "feedback_type": feedback_data.feedback_type.value,
                    "comment": feedback_data.comment,
                }
            ).eq("id", old_feedback.data[0]["id"]).execute()
            
            return FeedbackResponse(**update.data[0])
...
async def get_user_feedback(
    ...
    try:
        ...
        feedback_list = (
            [FeedbackResponse(**item) for item in result.data] if result.data else []
        )

        return FeedbackListResponse(feedback=feedback_list, total=len(feedback_list))
...
async def get_feedback_stats(
    ...
    try:
        ...
        thumbs_up_count = sum(
            1 for f in feedback_data if f["feedback_type"] == "thumbs_up"
        )
        thumbs_down_count = sum(
            1 for f in feedback_data if f["feedback_type"] == "thumbs_down"
        )

        positive_rate = (
            (thumbs_up_count / total_feedback * 100) if total_feedback > 0 else 0.0
        )
