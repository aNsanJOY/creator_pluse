...
from app.services.sources.base import SourceRegistry, BaseSourceConnector
...
class CrawlerService:
    ...
    async def crawl_source(self, source_id: str) -> dict:
        ...
        try:
            ...
            # Fetch content
            logger.info(f"Fetching content from source {source_id} (type: {source_type})")
            ...
            logger.info(f"Crawl completed for source {source_id}: {items_stored} items fetched")
            
            return {
                "success": True,
                "items_fetched": items_stored,
                "error": None
            }
    ...
    async def crawl_all_sources(self, user_id: Optional[str] = None) -> dict:
        ...
        try:
            ...
            query = self.supabase.table("sources").select("id").eq("status", SourceStatus.ACTIVE.value)
            ...
            logger.info(f"Crawl completed: {len(sources)} sources, {total_items} items, {len(errors)} errors")
